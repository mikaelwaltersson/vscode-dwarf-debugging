...
WasmRegisters::GetRegisterInfoAtIndex(size_t reg) {
  if (reg == 0) {
...
bool WasmUnwind::DoGetFrameInfoAtIndex(uint32_t frame_idx, lldb::addr_t &cfa,
                                       lldb::addr_t &pc,
...
WasmThread::CreateRegisterContextForFrame(lldb_private::StackFrame *frame) {
...
size_t WasmProcess::DoReadMemory(lldb::addr_t vm_addr, void *buf, size_t size,
...
SymbolFileWasmDWARF::CreateInstance(lldb::ObjectFileSP objfile_sp) {
...
Environment Host::GetEnvironment() { return {}; }
} // namespace lldb_private
