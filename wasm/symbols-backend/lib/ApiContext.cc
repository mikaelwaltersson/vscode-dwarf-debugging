...
SourceLocationToRawLocationResponse ApiContext::SourceLocationToRawLocation(
    std::string raw_module_id, std::string source_file, int32_t line_number,
...
ApiContext::RawLocationToSourceLocation(std::string raw_module_id,
                                        int32_t code_offset,
...
ApiContext::ListVariablesInScope(std::string raw_module_id, int32_t code_offset,
                                 int32_t inline_frame_index) {
...
GetInlinedFunctionRangesResponse
ApiContext::GetInlinedFunctionRanges(std::string raw_module_id,
                                     int32_t code_offset) {
...
GetInlinedCalleesRangesResponse
ApiContext::GetInlinedCalleesRanges(std::string raw_module_id,
                                    int32_t code_offset) {
...
void ApiContext::DeleteModule(llvm::StringRef id) { modules_.erase(id); }
...
llvm::Expected<std::vector<api::TypeInfo>>
ApiContext::GetApiTypeInfos(lldb_private::CompilerType type,
...
llvm::Optional<lldb_private::CompilerType>
ApiContext::GetTypeFromId(llvm::StringRef type_id) {
...
struct EvalVisitor {
  ...
  EvalVisitor(ApiContext &context, lldb_private::CompilerType type,
              llvm::Optional<size_t> address)
  ...
  template <typename T> api::EvaluateExpressionResponse MakeResponse(T v) {
...
static llvm::Expected<DebuggerProxy::WasmValue>
readWasmValue(const emscripten::val &value) {
...
llvm::Expected<DebuggerProxy::WasmValue>
DebuggerProxy::GetGlobal(size_t index) const {
  return readWasmValue(proxy_.call<emscripten::val>("getGlobal", index));
}
llvm::Expected<DebuggerProxy::WasmValue>
DebuggerProxy::GetLocal(size_t index) const {
  return readWasmValue(proxy_.call<emscripten::val>("getLocal", index));
}
llvm::Expected<DebuggerProxy::WasmValue>
DebuggerProxy::GetOperand(size_t index) const {
  return readWasmValue(proxy_.call<emscripten::val>("getOperand", index));
}

api::EvaluateExpressionResponse
ApiContext::EvaluateExpression(RawLocation location, std::string expression,
