...
public:
  ...
  llvm::Expected<size_t> ReadMemory(lldb::addr_t address, void *buffer,
                                    size_t size) const;
...
public:
  ...
  SourceLocationToRawLocation(std::string raw_module_id,
                              std::string source_file, int32_t line_number,
                              int32_t column_number) final;

  RawLocationToSourceLocationResponse
  RawLocationToSourceLocation(std::string raw_module_id, int32_t code_offset,
                              int32_t inline_frame_index) final;

  ListVariablesInScopeResponse
  ListVariablesInScope(std::string raw_module_id, int32_t code_offset,
                       int32_t inline_frame_index) final;
  ...
  GetInlinedFunctionRanges(std::string raw_module_id,
                           int32_t code_offset) override;

  GetInlinedCalleesRangesResponse
  GetInlinedCalleesRanges(std::string raw_module_id,
                          int32_t code_offset) override;
  ...
  EvaluateExpression(RawLocation location, std::string expression,
