...
template <typename T> static void initialize() { // NOLINT
  T::Initialize();
}
template <typename T> static void terminate() { // NOLINT
  T::Terminate();
}
...
public:
  ...
  DoCreateRegisterContextForFrame(lldb_private::StackFrame *frame) final {
...
public:
  ...
  struct WasmValueLoader {
    ...
    virtual llvm::Expected<api::DebuggerProxy::WasmValue>
