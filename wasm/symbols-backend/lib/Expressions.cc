...
struct WasmValueLoaderContext : SymbolFileWasmDWARF::WasmValueLoader {
  ...
  llvm::Expected<api::DebuggerProxy::WasmValue>
  LoadWASMValue(uint8_t storage_type, const lldb_private::DataExtractor &data,
...
llvm::Expected<ExpressionResult> InterpretExpression(
    const WasmModule &module, lldb_private::TypeSystem &type_system,
    lldb_private::SymbolContext &sc, size_t frame_offset,
    size_t inline_frame_index, lldb_private::Address addr,
    llvm::StringRef expression, const api::DebuggerProxy &proxy) {
  ...
  case lldb::BasicType::eBasicTypeLongLong: {
    ...
    if (auto integer_size = type.GetByteSize(nullptr)) {
      switch (*integer_size) {
      case 1:
        return ExpressionResult{type, static_cast<int8_t>(integer), address};
      case 2:
        return ExpressionResult{type, static_cast<int16_t>(integer), address};
      case 4:
        return ExpressionResult{type, static_cast<int32_t>(integer), address};
      case 8:
        return ExpressionResult{type, static_cast<int64_t>(integer), address};
      }
      break;
    }
  }
  ...
  case lldb::BasicType::eBasicTypeUnsignedLongLong: {
    ...
    if (auto integer_size = type.GetByteSize(nullptr)) {
      ...
      case 2:
        return ExpressionResult{type, static_cast<uint16_t>(integer), address};
      case 4:
        return ExpressionResult{type, static_cast<uint32_t>(integer), address};
      case 8:
        return ExpressionResult{type, static_cast<uint64_t>(integer), address};
      }
    }
    break;
  }
  ...
  case lldb::BasicType::eBasicTypeFloat:
    return ExpressionResult{type, result.GetFloat().convertToFloat(), address};
  case lldb::BasicType::eBasicTypeDouble:
    return ExpressionResult{type, result.GetFloat().convertToDouble(), address};
