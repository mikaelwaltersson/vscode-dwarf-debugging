...
namespace std {
template <> struct less<symbols_backend::SourceLocation> {
...
template <> struct less<symbols_backend::Variable> {
...
llvm::Expected<std::unique_ptr<WasmModule>>
WasmModule::CreateFromFile(llvm::StringRef path) {
...
uint32_t GetSymbolContextFromOffset(lldb_private::Module *module,
                                    lldb::addr_t offset, int inline_frame_index,
...
void GetVariablesFromOffset(lldb_private::Module *module, lldb::addr_t offset,
...
WasmModule::FindType(llvm::StringRef name) const {
...
llvm::SmallSet<SourceLocation, 1>
WasmModule::GetSourceLocationFromOffset(lldb::addr_t offset,
...
std::vector<int32_t>
WasmModule::GetMappedLines(llvm::StringRef file_path) const {
...
std::set<Variable>
WasmModule::GetVariablesInScope(lldb::addr_t offset,
                                int inline_frame_index) const {
...
llvm::Expected<ExpressionResult> WasmModule::InterpretExpression(
    lldb::addr_t frame_offset, uint32_t inline_frame_index,
    llvm::StringRef expression, const api::DebuggerProxy &proxy) const {
  lldb_private::SymbolContext sc;
  lldb_private::Address addr;
  auto resolved =
